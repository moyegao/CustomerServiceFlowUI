    ...
    <div class="dialog123">
      <el-dialog :close-on-click-modal="false" :title="dialogTitle" :visible.sync="addContent">
        <el-form :model="ruleForm" ref="form" :rules="rules" label-width="100px">
          ...
          <div class="my-input">
            <el-form-item label="主 办 人：" prop="sponsorUserName">
              <el-select @change="dealOutHelper2" v-model="ruleForm.sponsorUserName" clearable size="mini" style="width:100%">
                <el-option-group :key='group.id' v-for="group in options5" :label="group.label">
                  <el-option v-for="item in group.options" :key="item.id" :label="item.name" :value="item.id">
                  </el-option>
                </el-option-group>
...
export default {
  ...
  methods: {
    ...
    dealOutHelper2(val) {
      ...
      let options5 = JSON.parse(JSON.stringify(this.options5));
      // console.log(options5);
      this.ruleForm.sponsorUserName = '';
      for (let i = 0; i < options5.length; i++) {
        for (let j = 0; j < options5[i].options.length; j++) {
          if (val == options5[i].options[j].id) {
            this.ruleForm.sponsorUserName = options5[i].options[j].name;
          }
      ...
      //     for (let y in options5[x].options) {
      //       if (options5[x].options[y].id == val[i]) {
      //         this.ruleForm.sponsorUserName = options5[x].options[y].name;
      //         debugger
      //       }
      //     }
      //   }
    ...
    update(row) {
      ...
      this.id = row.id;
      if (row.taskState == '已办结') {
        this.$message({ message: '已办结任务不可修改！', type: 'warning' });
        return;
      }
      this.dialogTitle = '修改';
    ...
    delet(row) {
      if (row.taskState == '已办结') {
        this.$message({ message: '已办结任务不可删除！', type: 'warning' });
        return;
      }
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          var params = {
            ...
            sponsorUserName: that.ruleForm.sponsorUserName,
          ...
          for (let i = 0; i < that.options5.length; i++) {
            for (let j = 0; j < that.options5[i].options.length; j++) {
              if (that.ruleForm.sponsorUserName == that.options5[i].options[j].name) {
                params.sponsorUserId = that.options5[i].options[j].id;
              }
...
.privilegeModification td {
  padding: 0px 0px !important;
}
.privilegeModification .searchBox span {
  font-size: 0.16rem;
  // width: 100%;
}
.privilegeModification .searchBox .el-button {
  height: 0.3rem;
  background: #47b5e8;
}
.privilegeModification .tableBox {
  height: 82%;
  padding: 0 10px;
}
.privilegeModification .el-table {
  font-size: 15px;
}
.privilegeModification .my-input1 {
  margin-right: 0.1rem;
...
.privilegeModification .my-input {
  width: 49%;
  display: inline-block;
  .el-form-item {
    margin-bottom: 14px;
...
.privilegeModification .my-input .el-input {
  width: calc(100% - 80px);
}
.privilegeModification .my-input .el-select .el-input {
  width: 100%;
}
.privilegeModification .text1 {
  display: inline-block;
  width: 0.6rem;
}
.privilegeModification .text {
  text-align: right;
  display: inline-block;
  width: 0.8rem;
}
.privilegeModification .el-dialog__body {
  padding: 0px 20px;
  background-color: #fff;
}
.privilegeModification .dialog123 {
  box-sizing: content-box;
}
.privilegeModification .dialog123 .el-dialog {
  margin-top: 0.3rem !important;
  margin: 0 auto 0px;
  height: 6.65rem;
}
.privilegeModification .dialog123 .el-dialog .el-dialog__header {
...
.privilegeModification .dialog123 .el-dialog .el-dialog__body {
...
.privilegeModification .dialog123 .el-dialog .el-dialog__footer {
...
.privilegeModification .dialog123 .el-input--suffix .el-input__inner {
  padding-right: 26px;
  width: 100%;
}
.privilegeModification .planA {
  display: none;
}
.privilegeModification .planB {
  display: block;
  margin-top: 15px;
}
.privilegeModification .is-leaf {
  font-size: 18px;
}
/* 修改、删除 */
.privilegeModification .el-button--text {
  font-size: 0.14rem;
}
/* 查询、 增加 */
// background: #47b5e8;
.privilegeModification .el-button--mini {
  font-size: 16px;
}
.privilegeModification .searchBtn {
  background: #faa646 !important;
  border-color: #faa646 !important;
  // margin-right:3%;
}
.privilegeModification .el-form-item__content {
  margin-left: 110px !important;
}
.privilegeModification .el-form-item__label {
  width: 110px !important;
}
.privilegeModification .gocenter {
  width: 49%;
}
.privilegeModification .gocenter .el-form-item__label {
  text-align: center !important;
}
.privilegeModification .gocenter .el-textarea__inner {
  min-height: 72px !important;
}
.privilegeModification .textarea {
  width: 98.6%;
  // margin-left: 7%;
  .el-form-item {
    margin-bottom: 14px;
...
.privilegeModification .el-table__body tr td:nth-child(3) div {
  text-align: left;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.privilegeModification .el-table__row td:not(.is-hidden):last-child {
  right: -1px;
}
.privilegeModification .searchBox .el-range-separator {
  width: 8%;
}
.privilegeModification .block .el-input__inner {
  line-height: 22px;
}
input::-ms-clear,
input::-ms-reveal {
