<template>
  <div class="alltaskUrge" v-loading.fullscreen.lock="urgeLoading" element-loading-text="正在催办..." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
    <div class="alltaskDetail">
      ...
      <div v-bind:class="{searchBoxShow:show}" class="searchBox clear">
        ...
        <div class="my-input1" style="width:26%">
          <span>开始时间：</span>
          <div class="block" style="display:inline-block;width:75%;">
            <el-date-picker :editable="false" v-model="beginMonth" size="mini" type="month" unlink-panels format="yyyyMM" value-format="yyyyMM" placeholder="选择月" style="width:44%">
            </el-date-picker> 至
            <el-date-picker :editable="false" v-model="endMonth" size="mini" type="month" unlink-panels format="yyyyMM" value-format="yyyyMM" placeholder="选择月" style="width:44%">
            </el-date-picker>
          </div>
        </div>
        <el-button class="topbtn" type="primary" size="mini" @click="search" icon="el-icon-search"></el-button>
        <el-button type="primary" class="urgeBtn" size="mini" @click="taskUrge" style="float: right;margin-right:.1rem">催办</el-button>
        <div class="my-input1" style="width:19%;float: right;margin-right:.1rem">
          <span>催办类型：</span>
          <el-select v-model="urgeType" clearable size="mini" style="width:60%">
            <el-option label="办结催办" value="1"></el-option>
            <el-option label="汇报催办" value="2"></el-option>
          </el-select>
        </div>

        <!-- <el-button type="primary" size="mini" @click="attention">关注</el-button> -->
        <!-- <div :class="{planA: isActive, planB:isActive1}">
          <div class="my-input1" style="width:30%;margin-left:1%;">
            <span class="">任务时间：</span>
            <div class="block" style="display:inline-block;width:65%;">
      ...
      <div class="tableBox" id="tableBox" style="margin-top:10px;">
        <el-table stripe ref="tableList" @cell-click="handleClick" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" @selection-change="handleSelectionChange" :row-class-name="tableRowClassName" :data="tableData3" style="width: 100%; heihgt:92%;">
...
export default {
  ...
  methods: {
    ...
    taskUrge() {
      ...
      that.urgeLoading = true;
      let params = {
        remindType: this.urgeType,
        ids,
      }
      this.$common.commitParam(this, '/supervise/taskRemind/send.api', params, {
        success: function (res) {
          if (res.code === 0) {
            that.urgeLoading = false
            that.$message({
              message: '催办成功！',
              type: 'success'
            });
...
@media screen and (max-width: 1366px) {
  .alltaskUrge .el-dialog__body {
    padding: 0px 20px !important;
    height: 4.8rem !important;
  }
  .alltaskUrge .dialogBox > .el-dialog__wrapper > .el-dialog {
    width: 80% !important;
    margin: 0 auto;
    height: 5.5rem;
    margin-top: 0.38rem !important;
